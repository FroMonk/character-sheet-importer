using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

using Android.App;
using Android.Content;
using Android.OS;
using Android.Runtime;
using Android.Util;
using Android.Views;
using Android.Widget;
using Fragment = Android.Support.V4.App.Fragment;
using CharacterSheet.UI.Activities;
using CharacterSheet.UI.Views;
using CharacterSheet.UI.Dialogs;
using Android.Graphics;
using static CharacterSheet.UI.Helpers.DeviceHelper;
using CharacterSheet.Pathfinder;

namespace CharacterSheet.UI.Fragments.Feats
{
    public class FeatsFragment : Fragment
    {
        private const string KEY = "key";

        private MainActivity _activity;
        private LayoutInflater _inflater;
        
        private ScaledTextView _descriptionView;
        private ScaledTextView _titleView;
        private GenericListAdapter<Feat> _adapter;
        
        public static FeatsFragment NewInstance(string key)
        {
            FeatsFragment fragment = new FeatsFragment();
            Bundle bundle = new Bundle(1);
            bundle.PutString(KEY, key);
            fragment.Arguments = bundle;

            return fragment;
        }

        public override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);
        }
        
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            var key = Arguments.GetString(KEY);

            _activity = (MainActivity)Activity;
            _activity.CurrentTab = TabType.Feats;
            _activity.CurrentFeatsKey = key;
            _activity.UpdateOptionsMenu();

            _inflater = inflater;

            var selectedCharacter = _activity.Game.SelectedCharacter;

            var isPhone = _activity.FormatHelper.DeviceHelper.TypeOfDevice == DeviceType.Phone;

            var root = (ViewGroup)_inflater.Inflate(isPhone ? Resource.Layout.feats_fragment_min : Resource.Layout.feats_fragment, null);

            _titleView = (ScaledTextView)root.FindViewById(Resource.Id.title);
            _descriptionView = (ScaledTextView)root.FindViewById(Resource.Id.description);

            var listView = (ListView)root.FindViewById(Resource.Id.list);

            _adapter = new GenericListAdapter<Feat>(_activity, selectedCharacter.Feats.FeatsList[key]);
            listView.Adapter = _adapter;
            listView.ItemClick += List_Click;

            return root;
        }

        void List_Click(object sender, ListView.ItemClickEventArgs e)
        {
            var feat = _adapter[e.Position];

            if (_titleView != null && _descriptionView != null)
            {
                _titleView.SetText(feat.Name);
                _descriptionView.SetText(feat.Description);
            }
            else
            {
                new DescriptionDialog(_activity, _inflater, feat);
            }
        }
            
    }
}