...
namespace CharacterSheet.Pathfinder.XMLReader
{
    ...
    public class HeroLabFifthEditionXMLReader
    {
        private enum Region
        {
            AllSaves,
        ...
        private const string PLATINUM_PIECES = "platinum pieces";
        private const string GOLD_PIECES = "gold pieces";
        private const string ELECTRUM_PIECES = "electrum pieces";
        private const string SILVER_PIECES = "silver pieces";
        private const string COPPER_PIECES = "copper pieces";
        ...
        private const string COINS_TAG = "coins";
        ...
        private const string ABILITY_SCORES_TAG = "abilityscores";
        private const string ABILITY_SCORE_TAG = "abilityscore";
        private const string ABILITY_VALUE_TAG = "abilvalue";
        private const string ABILITY_BONUS_TAG = "abilbonus";
        ...
        private const string SAVING_THROW_TAG = "savingthrow";
        ...
        private const string IS_PROFICIENT_ATTRIBUTE = "isproficient";
        ...
        private const string MAX_DEX_BONUS_ATTRIBUTE = "maxdexbonus";
        ...
        private const string ABILITY_ABBREVIATION_ATTRIBUTE = "abilabbreviation";
        ...
        private const string STRENGTH_SAVE_VALUE = "strength";
        private const string DEXTERITY_SAVE_VALUE = "dexterity";
        private const string CONSTITUTION_SAVE_VALUE = "constitution";
        private const string INTELLIGENCE_SAVE_VALUE = "intelligence";
        private const string WISDOM_SAVE_VALUE = "wisdom";
        private const string CHARISMA_SAVE_VALUE = "charisma";
        ...
        private Dictionary<string, List<string>> _casterClassArchetypesList = new Dictionary<string, List<string>>
    {
        ...
        public HeroLabFifthEditionXMLReader()
        {
            _tempSpellList = new List<Spell>();
        }

        public CharacterStats parseCharacterXML(XmlReader xpp)
        {
            Check.ForNullArgument(xpp, nameof(xpp));
        ...
        private void AllocateSpells(CharacterStats characterStats)
        {
            ...
            var emptySpellClasses = _spells.SpellClasses.Select(x => x.Value).Where(y => y.Levels.Any(z => !z.Value.Spells.Any())).ToList();
            ...
            foreach (Spell spell in _tempSpellList)
            {
                if (!string.IsNullOrEmpty(spell.CasterClass))
                {
                    ...
                    if (_casterClassArchetypesList.ContainsKey(casterClassLower))
                    {
                        ...
                        var emptyClassNames = emptySpellClasses.Select(x => x.Name.ToLower()).ToList();

                        var matchingClasses = archetypes.Intersect(emptyClassNames).ToList();

                        if (matchingClasses.Count() == 1)
                        {
                            matchingCasterClass = emptySpellClasses.FirstOrDefault(x => x.Name.ToLower() == matchingClasses.FirstOrDefault());
                        }
                    }

                    if (matchingCasterClass == null && emptySpellClasses.Count(x => x.CasterClass.ToLower().Contains(casterClassLower)) == 1)
                    {
                        matchingCasterClass = emptySpellClasses.FirstOrDefault(x => x.CasterClass.ToLower().Contains(casterClassLower));
                    }
            ...
            emptySpellClasses = _spells.SpellClasses.Select(x => x.Value).Where(y => y.Levels.Any(z => !z.Value.Spells.Any())).ToList();

            if (emptySpellClasses.Count == 1)
            {
                ...
                {
                    var emptySpellClass = emptySpellClasses.FirstOrDefault();
        ...
        private void AbilityScoreEndTag()
        {
            ...
            if (_tags.Contains(_tagName))
            {
                switch (_tagName)
                {
                    ...
                    case CHARACTER_WEIGHT_TAG:
                        CharacterWeightTag();
                        break;
                    case ABILITY_SCORES_TAG:
                        AbilityScoresTag();
                        break;
                    case ABILITY_SCORE_TAG:
                        AbilityScoreTag();
                        break;
                    case ABILITY_VALUE_TAG:
                        AbilityValueTag();
                        break;
                    case ABILITY_BONUS_TAG:
                        AbilityBonusTag();
                        break;
                    case HEALTH_TAG:
                        HealthTag();
        ...
        private void CreateWeapon(Weapon weapon)
        {
            ...
            weapon.Critical = new Critical("20/x2");
        ...
        private void SkillTag()
        {
            ...
            _tempSkill.Ability = GetValue(ABILITY_ABBREVIATION_ATTRIBUTE);
            ...
            _tempSkill.IsProficient = string.Equals(GetValue(IS_PROFICIENT_ATTRIBUTE), "yes", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void SavingThrowTag()
        {
            ...
            _tempSavingThrow.IsProficient = string.Equals(GetValue(IS_PROFICIENT_ATTRIBUTE), "yes", StringComparison.OrdinalIgnoreCase);
        ...
        private void ArmorClassTag()
        {
            ...
            _ac.SetMaxDex(GetValue(MAX_DEX_BONUS_ATTRIBUTE));
        }
        ...
        private void CoinsTag()
        {
            var coins = GetValue(COUNT_ATTRIBUTE);

            switch (GetValue(NAME_ATTRIBUTE))
            {
                case PLATINUM_PIECES:
                    _bio.Money.SetPP(coins);
                    break;
                case GOLD_PIECES:
                    _bio.Money.SetGP(coins);
                    break;
                case ELECTRUM_PIECES:
                    _bio.Money.SetEP(coins);
                    break;
                case SILVER_PIECES:
                    _bio.Money.SetSP(coins);
                    break;
                case COPPER_PIECES:
                    _bio.Money.SetCP(coins);
                    break;
            }
        }
        ...
        {
            _bio.Money.SetValuables(GetValue(VALUABLES_ATTRIBUTE));
        }
